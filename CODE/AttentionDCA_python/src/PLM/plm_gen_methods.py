...
def generate_plm_vect(J,n_iter, init_sequence=500,beta=1,nb_PCA_comp=0,PCA_comp_list=np.array([]),J_PCA=None,beta_PCA=1):
    ...
    Generate N_seqs new sequences using PLM (random initialization by default)
    """
    batch_model = BatchSequencePLM(J, init_sequence,beta=beta,nb_PCA_comp=nb_PCA_comp,PCA_component_list=PCA_comp_list,J_tens_PCA=J_PCA,beta_PCA=beta_PCA)
...
def generate_plm_vect_n_save(save_dir, save_name, J, n_iter=1000, init_sequence=500,beta=1,nb_PCA_comp=0,PCA_comp_list=np.array([]),J_PCA=None,beta_PCA=1):
    """
    Generates a set of sequences using the PLM and saves them both as a numpy file and a text file containing the corresponding letter sequences.
    Saves:
    - A `.npy` file containing the generated sequences in numerical format.
    - A `.txt` file containing the generated sequences in letter format.
    """
    gen_sequences = generate_plm_vect(J, n_iter, init_sequence,beta=beta,nb_PCA_comp=nb_PCA_comp,PCA_comp_list=PCA_comp_list,J_PCA=J_PCA,beta_PCA=beta_PCA)
    gen_sequences_letters = [nums_to_letters(sequence,nb_PCA_comp) for sequence in gen_sequences]
...
def generate_PCA_coords(init_sequence, N_iter, J_PCA, nb_PCA_comp=2, beta_PCA=1, J=None, PCA_comp_list=None, beta=1, model=1):
    """
...
def generate_coords_n_save(save_dir, save_name, J, N_iter=100, init_sequence=None,beta=1,nb_PCA_comp=0,PCA_comp_list=np.array([]),J_PCA=None,beta_PCA=1, model=1):
    ...
    PCA_coords = generate_PCA_coords(init_sequence=init_sequence, N_iter=N_iter, J_PCA=J_PCA,
                                     ...
                                     beta_PCA=beta_PCA,
                                     J=J,
                                     PCA_comp_list=PCA_comp_list, beta=beta, model=model)

    PCA_coords = np.array(PCA_coords)
    print(PCA_coords.shape)  # Print shape of PCA coordinates
    print(f"Generated PCA coordinates (first 5): {PCA_coords[:5]}")  # Show first 5 coordinates
    #if nb_PCA_comp != 0:
