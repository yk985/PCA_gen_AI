...
def plot_pca_results(file_dir,file_name, target_coords = None, mac=True):
    ...
    if file_name is not None:
        ...
        for file_name in os.listdir(file_dir):
            if file_name.endswith('.npy'):
                ...
                plot_projected_pca_time(sequences_reference=train_sequences, sequences_to_project=gen_sequences, title=f"PCA Projection: Train vs {file_name}", Nbins=35,target_coords=target_coords)
        return 0
...
def plot_energy_from_seq(target, model=1, b=1, b_PCA=1, Nbins=35):
    Jtens = load_J_tensor(model=model)
    Jtens_PCA = None
    nb_PCA_comp = 2
    if model != 1:
    ...
    if model ==3:
        nb_PCA_comp = 1
    init_sequence_num = load_target_seq(target)
    ...
    plt.imshow(energies_2D, origin='lower', cmap='viridis_r', extent=(0, Nbins, 0, Nbins), aspect='auto')
