...
def trainerCondJ(n_epochs, H=32, d=23, Nbins=35, Ncomp=2, batch_size=1000, eta=0.005, lambd=0.001,
    ...
    q = int(Z.max()) + 1
    N, M = Z.shape
    Z=add_PCA_coords(Z.T,Nbins, n_comp_PCA=Ncomp).T
    W = W / W.sum()  # Normalize weights
    print(Z.shape)
...
def ar_trainer(n_epochs, H=32, d=23, batch_size=1000, eta=0.005, lambd=0.001,
            init_m=None, init_fun=np.random.randn,filename = None, structfile=None, verbose=True, savefile=None, losstype = 'without_J', index_last_domain1=0, H1=0, H2 =0,max_gap_frac=0.9):   
...
def trainer_PCA_comp_2_model(n_epochs,Q,K,V ,H=32, d=23, batch_size=1000, eta=0.005, lambd=0.001,
    ...
    print("Z deviiiice",Z1_test.device)
    ...
    num_workers = 4 if device.type == 'cuda' else 0  # Adjust if necessary
    train_dataset = TensorDataset(Z1_train.cpu(),Z2_train.cpu(), w_train.cpu())
    test_dataset = TensorDataset(Z1_test.cpu(), Z2_test.cpu() ,w_test.cpu())
    print(2)
    ...
    def validate(epoch, train_losses=[]):
        ...
        with torch.no_grad():
            for batch_z1, batch_z2 ,batch_w in test_loader:
                batch_z1 = batch_z1.to(device, non_blocking=True)
                batch_z2 = batch_z2.to(device, non_blocking=True)
                batch_w = batch_w.to(device, non_blocking=True)
                loss = model(Q,K,V,batch_z1.T.contiguous(),batch_z2.T.contiguous() , batch_w)
                val_losses.append(loss.item())
...
def trainer_PCA_comp_2_model_test_cross(n_epochs, H=32, d=23, batch_size=1000, eta=0.005, lambd=0.001,
            init_m=None, init_fun=np.random.randn,filename = None, structfile=None, verbose=True, savefile=None, losstype = 'without_J', index_last_domain1=0, H1=0, H2 =0,max_gap_frac=0.9,nb_bins_PCA=35):   
    ...
    Z1=Z[:-2,:]
    Z2=Z[-2:,:]
    Z2=Z1.copy() #à changer
    W = W / W.sum()  # Normalize weights
    q1 = int(Z1.max()) + 1  # Assuming Z contains 0-based indices
    q2 = q1 # à changer aussi 
    N1, M = Z1.shape
    N2, _ = Z2.shape
...
def trainer_PCA_comp_2_model_once(n_epochs, H=32, d1=23,H_PCA=32,d2=23, batch_size=1000, eta=0.005, lambd=0.001,
    ...
    return model
def trainer_PCA_comp_2_model_flat(n_epochs, H=32, d=23, batch_size=1000, eta=0.005, lambd=0.001,
            init_m=None, init_fun=np.random.randn,filename = None, structfile=None, verbose=True, savefile=None, losstype = 'without_J', index_last_domain1=0, H1=0, H2 =0,max_gap_frac=0.9,nb_bins_PCA=35):   

    Z, W = quickread(filename,max_gap_frac=max_gap_frac)
    Z=add_coords_flat(Z.T,nb_bins_PCA).T
    Z1=Z[:-1,:]
    Z2=Z[-1:,:]
    W = W / W.sum()  # Normalize weights
    q1 = int(Z1.max()) + 1  # Assuming Z contains 0-based indices
    q2 = nb_bins_PCA*nb_bins_PCA # à checker avec marzio pour index 0 ou pas 
    N1, M = Z1.shape
    N2, _ = Z2.shape
